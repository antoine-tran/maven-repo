 ...
 * @since 1.0
 */
public abstract class StochasticMinimizer<T extends DoubleValuedFunction> implements Minimizer<T>, HasEvaluators
{
	...
	protected int k;
	// todo [cdm 2011]: Having this be static is CLEARLY wrong
	...
	public abstract IntDoublePair tune(DoubleValuedFunction function,double[] initial, long msPerTest);

	public double tuneDouble(DoubleValuedFunction function, double[] initial, long msPerTest,PropertySetter<Double> ps,double lower,double upper){
		return this.tuneDouble(function, initial, msPerTest, ps, lower, upper, 1e-3*Math.abs(upper-lower));
	}

	public double tuneDouble(DoubleValuedFunction function, double[] initial, long msPerTest,PropertySetter<Double> ps,double lower,double upper,double TOL){
		...
		List<DoubleDoublePair> res = new ArrayList<DoubleDoublePair>();
		DoubleDoublePair best = new DoubleDoublePair(lower,Double.POSITIVE_INFINITY); //this is set to lower because the first it will always use the lower first, so it has to be best
		DoubleDoublePair low = new DoubleDoublePair(lower,Double.POSITIVE_INFINITY);
		DoubleDoublePair high = new DoubleDoublePair(upper,Double.POSITIVE_INFINITY);
		DoubleDoublePair cur = new DoubleDoublePair();
		DoubleDoublePair tmp = new DoubleDoublePair();
		...
		do{
			...
			if(queue.size() != 0){
				cur.setFirst(queue.remove(0));
			}else{
				cur.setFirst(0.5*( low.first() + high.first()));
			}
			...
			if(Double.isNaN( xtest[0] ) ){
				cur.setSecond(Double.POSITIVE_INFINITY);
			} else {
				cur.setSecond(dfunction.valueAt(xtest));
			}
			...
			res.add(new DoubleDoublePair(cur.first(),cur.second()));
	...
	private static void copyPair(DoubleDoublePair from, DoubleDoublePair to){
		to.setFirst(from.first());
		to.setSecond(from.second());
	}
	...
	public double tuneGain(DoubleValuedFunction function, double[] initial, long msPerTest, double lower, double upper){
	...
	public int  tuneBatch(DoubleValuedFunction function, double[] initial, long msPerTest, int bStart){
	...
	public IntDoublePair tune(DoubleValuedFunction function, double[] initial, long msPerTest,List<Integer> batchSizes, List<Double> gains){
		...
		return new IntDoublePair(bOpt,gOpt);
	...
	@Override
	public double[] minimize(DoubleValuedFunction function, double functionTolerance, double[] initial, int maxIterations) {
