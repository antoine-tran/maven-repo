...
public class QNMinimizer implements Minimizer<DiffFunction>, HasEvaluators {
  ...
  private DoubleValuedFunction monitor = null;
  ...
  public QNMinimizer(DoubleValuedFunction monitor) {
  ...
  public QNMinimizer(DoubleValuedFunction monitor, int m) {
    this.monitor = monitor;
    mem = m;
  }

  public QNMinimizer(DoubleValuedFunction monitor, int m, boolean useRobustOptions) {
    this.monitor = monitor;
    mem = m;
    if (useRobustOptions) {
      this.setRobustOptions();
  ...
  public class Record {
    ...
    private int maxSize = 100; // This will control the number of func values /
                                // gradients to retain.
    private DoubleValuedFunction mon = null;
    ...
    public Record(boolean beQuiet, DoubleValuedFunction monitor) {
      this.quiet = beQuiet;
      this.mon = monitor;
    }

    public Record(boolean beQuiet, DoubleValuedFunction monitor, double tolerance) {
      this.quiet = beQuiet;
      this.mon = monitor;
      this.TOL = tolerance;
    }
   ...
   * conditions.
   */
  private double[] lineSearchBacktrack(DoubleValuedFunction func, double[] dir, double[] x,
