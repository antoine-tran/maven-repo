...
public class DoubleSet {
  ...
  public boolean forceAdd(double v) {
  ...
  protected int indexOf(double v) {
  ...
  protected void shrink() {
    if(numElements<data.length/2 && data.length>300) {    
      DoubleSet result=new DoubleSet(numElements+100);
  ...
  public void trim() {
	if(numElements<data.length) {
	  DoubleSet result=new DoubleSet(numElements);
	  result.addUnique(this);
      this.data=result.data;
      this.isThere=result.isThere;
      this.lastIndex=result.lastIndex;
  ...
  public boolean trimToSize(int minSize) {
	if(numElements<=minSize) {
	  DoubleSet result=new DoubleSet(minSize);
	  result.addUnique(this);
      this.data=result.data;
      this.isThere=result.isThere;
      this.lastIndex=result.lastIndex;
  ...
  /** Removes an item*/
  protected double removeIndex(int index) {
  ...
  public static DoubleSet of(double... initial) {
    DoubleSet result=new DoubleSet();
    for(double i : initial) result.add(i);
    return(result);
  }

  /** Creates a new IntSet by copying the given set*/
  public DoubleSet(DoubleSet copy) {
    setTo(copy);
  }

  /** Creates a new IntSet by copying the given bit set*/
  public DoubleSet(BitSet copy) {
  ...
  public void setTo(DoubleSet copy) {
    if(copy==this) return;
    clear(copy.size());
    addUnique(copy);
  }
  ...
  public boolean removeAll(DoubleSet c) {
  ...
  public boolean addAll(double[] c) {
    ...
    for(double i : c) returnValue|=add(i);
    return(returnValue);
  }

  /** Adds all elements*/
  public boolean addAll(DoubleSet s) {
    boolean returnValue=false;
  ...
  public boolean addUnique(double[] c) {
    boolean returnValue=false;
    for(double i : c) returnValue|=forceAdd(i);
    return(returnValue);
  }

  /** Adds all elements, skip checking duplicates */
  public boolean addUnique(DoubleSet s) {
    boolean returnValue=false;
    for(int index=0;index<=s.lastIndex;index++) {
  ...
  public boolean removeAll(double[] c) {
    boolean result=false;
    for(double o : c) result|=remove(o);
    return result;
  }

  /** Adds all elements, returns THIS*/  
  public DoubleSet enhancedBy(DoubleSet addMe) {
    addAll(addMe);
    return(this);
  }

  /** Adds addMe, returns THIS*/
  public DoubleSet enhancedBy(double addMe) {
    add(addMe);
    return(this);
  }
  
  /** Removes the given elements, returns THIS*/
  public DoubleSet shrunkBy(DoubleSet addMe) {
    removeAll(addMe);
    return(this);
  }
  
  /** Removes the given element, returns THIS*/
  public DoubleSet shrunkBy(double removeMe) {
    remove(removeMe);
    return(this);
  }
