...
public class ScaledSGDMinimizer extends StochasticMinimizer {
  ...
  public void serializeWeights(String serializePath,double[] weights,double[] diag) {
    ...
    try {
      ...
      FileUtility.serialize(out, serializePath);
    } catch (Exception e) {
  ...
  public double[] getWeights(String loadPath) throws Exception {

    System.err.println("Loading weights from " + loadPath + "...");
    double[] wt;
    Weight w;

    w = (Weight) FileUtility.read(loadPath);
    wt = w.w;
  ...
  public double[] getDiag(String loadPath) throws Exception {
