...
public class ResultIterator<T> extends PeekIterator<T> {  
	...
	/** ResultWrapper for byte array */
	public static final ResultWrapper<byte[]> BytesWrapper=new ResultWrapper<byte[]>() {		
		public byte[] wrap(ResultSet r) throws Exception {
			return r.getBytes(1);
		}
	};

	/** ResultWrapper that suppport different encoding (default UTF-8) */
	public static final class InternationalStringWrapper implements ResultWrapper<String> {
		private String encoding = "UTF-8";
		
		private InternationalStringWrapper() {
		}
		
		public static InternationalStringWrapper instance() {
			return new InternationalStringWrapper();
		}
